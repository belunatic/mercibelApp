...
const EditCustomer = () => {
  ...
  useEffect(() => {
    //get data from serve and populate the fields
    const getCustomerData = async () => {
      try {
        const data = await axios.get(`http://localhost:5000/customers/${id}`);
        console.log(data.data);
        setFormData((prevFormData) => ({ ...prevFormData, ...data.data }));
      } catch (error) {
        console.log("Error fetching customer data:", error);
        toastMessage("Error fetching customer data", false);
      }
    };
  ...
  const updateCustomer = async () => {
    try {
      const res = await axios.put(`http://localhost:5000/customers/${id}`, {
        ...formData,
